...
try {
  ...
  chrome.contextMenus.create( { title: "Посмотреть определения", contexts:["selection"], onclick: onClickRB });

  function onClickRB(info, tab) {
    
    //alert(info.selectionText);
    //chrome.tabs.sendRequest(tab.id, any request, function responseCallback);
    try {
      chrome.tabs.sendMessage( tab.id, {msg: "cbContextMenu", txt: info.selectionText}, null, function(response) {
        //cb(response);
      });
    } catch(ex) {
      alert("onClickRB: " + ex);
  ...
  chrome[b].onMessage.addListener(
    function(request, sender, sendResponse) {
      try {
        ...
        else if (request.msg == "getGTermGGlossaryListByText")
        {
          xhrData2.open("GET", conf.UrlService + conf.GetGTermGGlossaryListByText + request.text + "&" + request.ft, false);
          xhrData2.send();
          sendResponse(data2);
          return true;
        }
        else if (request.msg == "getGTermGGlossaryDescriptionByID")
        {
          xhrData2.open("GET", conf.UrlService + conf.GetGTermGGlossaryDescriptionByID + request.id, false);
          xhrData2.send();
          sendResponse(data2);
          return true;
        }
